...
test('Fluxo de Reserva - Cenario Positivo', async ({ page }) => {
  ...
  // Seleciona o destinto como London 
  ...
  await page.locator('#cardType').selectOption('amex');                         // seleciona a bandeira do cartao
  await page.getByRole('checkbox', { name: 'Remember me' }).check(); // Seleciona o check box

  // Muda de página

  await page.getByRole('button', { name: 'Purchase Flight' }).click(); // clica no botao Purchase Flight
